...
app.controller('MetricCtl', ['$scope', '$stateParams', 'MetricService', '$interval', '$timeout',
    function ($scope, $stateParams, MetricService, $interval, $timeout) {
        ...
        $scope.loading = false;
        $scope.custom = false;
        $scope.timeScopeShow = true;
        $scope.quick = false;
        $scope.charts = [];
        $scope.startTime = new Date();
        $scope.endTime = new Date();
        $scope.startTime.setMinutes($scope.endTime.getMinutes() - $scope.timeInterval);
        ...
        $scope.quickOnSetTime = function (calculate, offset) {
            if (calculate === 0) {
                ...
                $scope.startTime.setMinutes(offsetTime - $scope.timeInterval);
            }
            queryIdentityDatas();
        }
        ...
        $scope.refreshInterval = 1000 * $scope.refreshItems[0].val;

        $scope.timeScopeItems = [
            {val: 5, text: "5分钟"},
            {val: 10, text: "10分钟"},
            {val: 15, text: "15分钟"},
            {val: 30, text: "30分钟"},
        ...
        $scope.onTimeScopeIntervalChange = (item) => {
            $scope.timeInterval = item.val;
            reInitIdentityDatas();
        };
